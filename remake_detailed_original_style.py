...
def extract_content(text):
    # Extract Definitions for Grid Layout
    concepts = []
    ...
    # Fallback concepts
    if len(concepts) < 2:
        concepts = [{"title": "Componente", "desc": "Elemento visual fundamental do Swing."}]

    # Extract Code Snippets
    code_snippets = []
    paragraphs = text.split('\n\n')
    for p in paragraphs:
        if "new " in p and ";" in p and ("J" in p or "public" in p) and len(p) > 30:
             # Basic cleanup for code display
             code = p.strip().replace("    ", "  ")
             code_snippets.append(code)
    
    # Ensure at least one code snippet
    if not code_snippets:
        code_snippets.append("// Exemplo genérico\nComponent c = new Component();")

    return concepts, code_snippets[:2] # Return top 2 snippets

def generate_lesson_html(ch_num, filename, title, subtitle, raw_text):
    concepts, code_examples = extract_content(raw_text)
    
    # Generate Grid Items HTML (Lesson 01 Style)
    grid_html = ""
    for c in concepts:
        grid_html += f"""
                <div class="grid-item">
    ...
    # Generate Code Example HTML
    example_html = ""
    for code in code_examples:
        example_html += f"""
            <div style="background: #2d2d2d; color: #f8f8f2; padding: 15px; border-radius: 5px; margin-top: 10px; font-family: monospace; font-size: 0.9rem;">
                {code}
            </div>
        """
...
<body>
    ...
    <div class="slide-container">
        ...
        <div class="slide">
            ...
            <p>Veja abaixo exemplos extraídos do capítulo:</p>
            {example_html}
        </div>
...
try:
    ...
    for ch_num, (filename, title, subtitle) in lessons_map.items():
        print(f"Recriando Aula Detalhada (Visual Original): {filename}...")
        
        start_page = chapter_starts.get(ch_num)
        if not start_page: continue
        
        # Read enough pages to find code examples
        raw_text = ""
    ...
    print("Sucesso! Aulas 04-15 recriadas com detalhes e estilo visual consistente.")
